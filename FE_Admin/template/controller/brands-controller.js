...
window.BrandsController = function ($scope, $http, $window, $rootScope) {
  ...
  $scope.hienThi = function (pageNo, sizePage) {
    ...
    $http.get(apiUrl, headers).then(
      function (response) {
        ...
        $scope.listBrands = response.data.brandsList;
        $scope.totalPage = response.data.totalPages;
        $scope.lastIndex = $scope.listBrands[$scope.listBrands.length - 1].id;
  ...
  $scope.removeBrands = function (event, item) {
    event.preventDefault();

    console.log(item);
    let brandsId = item.id;
    let api = apiURL + "admin/Brands/delete/" + brandsId;
  ...
  $scope.addBrands = function (event) {
    event.preventDefault();
    console.log($scope.formbrands);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.post(apiAdmin + "Brands" + "/insert", JSON.stringify($scope.formbrands), headers)
  ...
  // update
  $scope.UpdateBrands = function (event) {
    event.preventDefault();
    console.log($scope.formBrandsUpdate);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.put(apiAdmin + "Brands/update", JSON.stringify($scope.formBrandsUpdate), headers)
          .then(function (response) {
            console.log("Success Response:", response.data); // Assuming the data property contains the relevant information
            Swal.fire({
              icon: 'success',
      ...
      try {
        ...
        worksheet.eachRow((row, index) => {
          if (index > 1) {
            ...
            $http.post(apiAdmin + "Brands" + "/insert", JSON.stringify(brands), headers)
              .then(function (response) {
                if (!$scope.errorShown) {
                  Swal.fire({
                    icon: "success",
