window.CategoryController = function ($scope, $http, $window) {
  ...
  $scope.hienThi = function (pageNo, sizePage) {
    let apiUrl = apiAdmin + "admin/Category" + "?pageNo=" + pageNo + "&sizePage=" + sizePage;
    $http.get(apiUrl, headers).then(
      function (response) {
        // Kiểm tra dữ liệu có được in ra không
        $scope.listCategory = response.data.categoryList;
        $scope.totalPage = response.data.totalPages;
        $scope.lastIndex = $scope.listCategory[$scope.listCategory.length - 1].id;
  ...
  // }
  //   };
  $scope.removeCategory = function (event, item) {
    event.preventDefault();

    console.log(item);
    let categoryId = item.id;
    let api = apiAdmin + "Category/delete/" + categoryId;
  ...
  $scope.addCategory = function (event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.post(apiAdmin + "Category/insert", JSON.stringify($scope.request), headers)
          .then(function (response) {
  ...
  $scope.formmCategoryUpdate = {}; // Khởi tạo biểu mẫu
  ...
  $scope.toggleFormUpdate = function (event, index) {
    ...
    if ($scope.activeIndex === index && $scope.showFormUpdate) {
      ...
      $scope.formCategoryUpdate.id = category.id;
      $scope.formCategoryUpdate.name = category.name;
      $scope.formCategoryUpdate.status = category.status;
  ...
  $scope.updateCategory = function (event) {
    event.preventDefault();
    console.log($scope.formCategoryUpdate);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.put(apiAdmin + "Category/update", JSON.stringify($scope.formCategoryUpdate), headers)
          .then(function (response) {
            console.log("Success Response:", response.data); // Assuming the data property contains the relevant information
            Swal.fire({
              icon: 'success',
      ...
      try {
        ...
        worksheet.eachRow((row, index) => {
          if (index > 1) {
            ...
            $http.post(apiAdmin + "Category/insert", JSON.stringify(category), headers)
              .then(function (response) {
                if (!$scope.errorShown) {
                  $scope.hienThi($scope.pageNo, $scope.sizePage);
                  Swal.fire({
