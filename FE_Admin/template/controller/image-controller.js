...
window.ImageController = function ($scope, $http, $window, $timeout) {
  ...
  $scope.hienThi = function (pageNo, sizePage) {
    ...
    $http.get(apiUrl, headers).then(
      function (response) {
        ...
        $scope.listImage = response.data.imageList;
        $scope.totalPage = response.data.totalPages;
        $scope.lastIndex = $scope.listImage[$scope.listImage.length - 1].id;
  ...
  $scope.fileChanged = function (element) {
    $scope.$apply(function () {
      ...
      $scope.formImage.link = fileInput.files[0].name;
    });
  };
  ...
  $scope.addImage = function (event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $http.post(apiAdmin + "Image" + "/insert", JSON.stringify($scope.formImage), headers)
    ...
    if (item && $scope.activeItem === item && $scope.showFormUpdate) {
      ...
      $scope.fileChanged = function (element) {
        $scope.$apply(function () {
          ...
          $scope.formImageUpdate.link = updateInput.files[0].name;
        });
      };
      $scope.formImageUpdate = {
        id: item.id,
        ...
        worksheet.eachRow((row, index) => {
          if (index > 1) {
            let image = {
              ...
              link: row.getCell(2).value,
              product: row.getCell(3).value,
            };
            $http
              .post(
                apiAdmin + "Image" + "/insert",
  ...
  $scope.exportToExcel = function () {
    // Lấy bảng theo ID
    var table = document.getElementById("ImageTable"); // Thay id table bảng của bạn vào đây
  ...
  $scope.selectAllChanged = function () {
    ...
    angular.forEach($scope.listImage, function (item) {
      item.isSelected = $scope.selectAllCheckbox;
    });
  };

  $scope.deleteAll = function () {
    var selectedItems = $scope.listImage.filter(function (item) {
